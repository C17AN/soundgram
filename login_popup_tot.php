...
<script type="text/javascript">
	...
	// 210607 문상혁 추가 - ios에서 애플아이디로 로그인 시 id를못잡아서
	var id = '';
	var device_app_id = 0;
    
	// 210524 SignInWithApple from iOS - 문상혁 추가
	// iOS에서 애플로그인 성공시 호출될 함수
	function signInWithApple(userIdentifier, fullName, email, uuid, device_app_id, tot_id, apppage_id) {
        ...
        // 추가정보입력 후 회원가입 또는 로그인 21.06.04
		console.log(device_app_id);
        var profile = "default/profile_none.png";
		
		console.log(fullName, userIdentifier, uuid, device_app_id, tot_id, apppage_id);
		id = `ap_${userIdentifier}`;
        getSNSUser(fullName, id, sha256(id), "", fullName, 4, uuid, device_app_id, tot_id, profile,"");
	}

	
	function getSNSUser(name, id, password, tel, nick, snstype, uuid, device_app_id, tot_id, profile, access_token) {
        var rcode="";
            ...
            success: function(data) {
                ...
                if(returnCode=="join_complete") {
                    if(isiplayer=="true") {
                        ...
                        //카카오 로그인의 경우 AccessToken을 입력해줘야됨.
                        if(snstype==1) {
                            //카카오 로그인 관련
                            Kakao.Auth.setAccessToken(access_token);
                        }

                        opener.loginComplete(data, "_login");
                    }
                    ...
                    var pageload = window.setInterval(function() {
                        if($("div.container_popup > div").length==7) {
                            ...
                            $("div.container_popup > div#"+cuid+"_h").removeClass("hidden");

                            // 애플로그인시에 padding값 수정
                            if(snstype==4) {
                                $("div#"+cuid+"_h").css({"padding":"25% 0 0 0"})
                            }
                        }
                    },100);
                }
...
<?php
	...
	define('NAVER_CALLBACK_URL', "https://".$hosturl.'/login_popup_tot.php?logintype=naver&device_app_id='.$device_app_id.'&tot_id='.$tot_id.'&uuid='.$uuid.'&isiplayer='.$isiplayer.'&diskpath='.$diskpath.'&diskid='.$diskid); // 네이버 로그인 접근토큰 요청 예제 
    ...
    if($logintype!=null) {
        ...
        if($logintype=="kakao") {
            ...
            // $callbacURI = urlencode("https://".$hosturl."/login_popup.php?logintype=kakao&albumid=".$albumid."&device_app_id=".$device_app_id."&uuid=".$uuid."&isiplayer=".$isiplayer); // 본인의 Call Back URL을 입력해주세요
            // $callbacURI = urlencode($server_url."/login_popup.php"); // 본인의 Call Back URL을 입력해주세요
            $callbacURI = urlencode("https://".$hosturl."/login_popup_tot.php"); // 본인의 Call Back URL을 입력해주세요
            // API 요청 URL
            ...
            if($kakao_responseArr['id']) {
                ...
                $profile = str_replace("http","https",$kakao_responseArr['properties']['thumbnail_image']);
...
?>
                <script type="text/javascript">
                    ...
                    // 회원가입인지 로그인인지 체크
                    getSNSUser(name, id, password, tel, nick, 1, uuid, device_app_id, tot_id, profile, access_token);
                </script>
<?php
            }
        }
...
?>
                <script type="text/javascript">
                    ...
                    // 회원가입인지 로그인인지 체크
                    getSNSUser(name, id, password, tel, nick, 2, uuid, device_app_id, tot_id, profile, access_token);
                </script>
<?php
                } 
                else { 
            ...
            $device_app_id = $param_arr[1];
            $uuid = $param_arr[2];
            $isiplayer = $param_arr[3];
            $diskpath = $param_arr[4];
            $diskid = $param_arr[5];
            ...
            define('GOOGLE_REDIRECT_URL', 'https://devdisk.soundgram.co.kr/login_popup_tot.php');
...
?>
                <script type="text/javascript">
                    ...
                    // 회원가입인지 로그인인지 체크
                    getSNSUser(name, id, password, tel, nick, 5, uuid, device_app_id, profile, access_token);
                </script>
<?php
...
<head>
    ...
    <script type="text/javascript">
        ...
        var cuid = "";
        if(isiplayer=="true") {
            var value = jQuery.param({"tot_id":tot_id});
            $.ajax({
                type : "POST",
                url : "api/getcuid.php",
                data : value,
        ...
        $(window).load(function() {
            ...
            for(var cu=0; cu<cuarr.length; cu++) {
                $("div.container_popup").append(getHTMLPageData(""+cuid+"_"+cuarr[cu]));
				if(ostype != 2) {
					console.log("ostype : ", ostype);
					// document.getElementById('li-apple').style.display = 'none';
                    $("li#li-apple").hide();
                }
                ...
                // else if(cu==1) {
                if(cu==0) {
                    $("div#"+cuid+"_b > div.mb_content > button.btn_bora").children("img").attr("src","/images/"+cuid+"_ic_login_white.png");

                    var naver_url = '<?=$naver_url?>';
                    $("a#naverIdLogin_loginButton").attr("href", naver_url);
                }
                else if(cu==1) {
                    $("div#"+cuid+"_c > div.mb_content > button.btn_bora").children("img").attr("src","/images/"+cuid+"_ic_write.png");
                }
                else if(cu==2) {
                    $("div#"+cuid+"_d").addClass("hidden");
                }
                else if(cu==3) {
                    $("div#"+cuid+"_e > div.mb_content > button#srhid").children("img").attr("src","/images/"+cuid+"_ic_idcheck.png");
                    $("div#"+cuid+"_e > div.mb_content > button#temp_pass").children("img").attr("src","/images/"+cuid+"_ic_pwcheck.png");
                }
                else if(cu==4) {
                    $("div#"+cuid+"_f > div.mb_content > button.btn_bora").children("img").attr("src","/images/"+cuid+"_ic_write.png");
                }
                else if(cu==5) {
                    $("div#"+cuid+"_g > div.profile > button#camera > img").attr("src","/images/"+cuid+"_ic_photo.png");
                    $("div#"+cuid+"_g > div.mb_content > ul.mb_profile > li.nick > button#changenick > img").attr("src","/images/"+cuid+"_ic_write.png");
                    $("div#"+cuid+"_g > div.mb_content > ul.mb_profile > li.nick > button#changetel > img").attr("src","/images/"+cuid+"_ic_write.png");
                    $("div#"+cuid+"_g > div.mb_content > button#changepass > img").attr("src","/images/"+cuid+"_ic_write.png");
                    $("div#"+cuid+"_g > div.mb_content > button#logout > img").attr("src","/images/"+cuid+"_ic_logout.png");
                    ...
                    if(isiplayer=="true") {
                        ...
                        if(opener.soundgramApi.loginoutflag=="1") {
							let account = opener.soundgramApi.account.length > 20 
								? opener.soundgramApi.account.substr(0,20) + "..." 
								: opener.soundgramApi.account;
                            
                            $("div.container_popup").children().addClass("hidden");
                            $("div#"+cuid+"_g").removeClass("hidden");
		...
		function EventGather() {
            $("div#"+cuid+"_a > div.mb_content > button.btn_boraline").click(function() {
                $("div#"+cuid+"_a").addClass("hidden");
                $("div#"+cuid+"_b").removeClass("hidden");
            ...
            $("div#"+cuid+"_b > div.mb_content > button#join").click(function() {
                $("div#"+cuid+"_b").addClass("hidden");
                $("div#"+cuid+"_c").removeClass("hidden");
            ...
            $("div#"+cuid+"_b > div.mb_content > button.btn_bora").click(function() {
                ...
                var value = jQuery.param({"id":id, "passwd":sha256(password), "uuid":uuid, "type":"L"});
                // console.log(value);
                $.ajax({
                    type : "POST",
                    url : "api/login_tot.php",
                    data : value,
                    dataType: "json",
                    success: function(data) {
                        console.log(data);
            ...
            $("div#"+cuid+"_c > div.mb_content > button.btn_bora").click(function() {
            ...
            $("div.mb_title, div.alert, button#goJoin, button#temp_pass").click(function(e) {
                $("div#"+cuid+"_c").animate({"padding":"20% 0 0 0"}, 400);
                $("div#"+cuid+"_f").animate({"padding":"20% 0 0 0"}, 400);
            ...
            $("div.container_popup").click(function(e) {
                if($("div#"+cuid+"_c").is(":visible")) {
                    if(e.target.className=="input_style" || e.target.className=="mb_form") return false;
                    else {
                        $("div#"+cuid+"_c").animate({"padding":"20% 0 0 0"}, 400);
                    }
                }
                else if($("div#"+cuid+"_g").is(":visible")) {
                    if(e.target.className=="input_style" || e.target.className=="mb_form") return false;
                    else {
                        $("div#"+cuid+"_g").animate({"padding":"12% 0 0 0"}, 400);
                    }
                }
                else if($("div#"+cuid+"_f").is(":visible")) {
                    if(e.target.className=="input_style" || e.target.className=="mb_form") return false;
                    else {
                        $("div#"+cuid+"_f").animate({"padding":"20% 0 0 0"}, 400);
                    }
            ...
            $("div#member_input > input").focusin(function(e) {
                ...
                if((idx>0&&idx<8) || idx==10) {
                    ...
                    $("div#"+cuid+"_c").animate({"padding":padding}, 400);
            ...
            $("div#"+cuid+"_b > div.mb_content > ul > li > a").click(function() {
                var id = $(this).attr("id");
                
                //카카오 로그인
                if(id=="kakao") {
                    var param = ["kakao",device_app_id,tot_id,uuid,isiplayer,diskpath,diskid];
                    Kakao.Auth.authorize({
                        redirectUri: server_url+'/login_popup_tot.php',
                        state: param.join(","),
                        throughTalk: false,
                    });
                }
                    ...
                    var param = ["google",device_app_id,uuid,isiplayer,diskpath,diskid];

                    location.href
                    = "https://accounts.google.com/o/oauth2/auth?client_id="+
                        "328053066691-3jcqkuq4g8qr56af8jgh2g510rne7mnc.apps.googleusercontent.com"+
                        "&redirect_uri=https://devdisk.soundgram.co.kr/login_popup_tot.php"+
                        "&state="+param.join(",")+ 
            ...
            $("button#srhidpass").click(function() {
                ...
                $("div#"+cuid+"_e").removeClass("hidden");
                effect("div#"+cuid+"_e");
            });
            ...
            $("button#srhid, button#temp_pass") .click(function() {
                ...
                if(thisbutton=="srhid") {
                    ...
                    var value = jQuery.param({"account":searchid, "phone":searchtel});
                    $.ajax({
                        type : "POST",
                        ...
                        success: function(data) {
                            if(data.returnCode=="notuser") {
                                ...
                                value = jQuery.param({"account":searchid, "password":sha256(password)});
                                $.ajax({
                                    type : "POST",
            ...
            $("#accessterms").click(function() {
                $("div.container_popup").children("div").addClass("hidden");
                $("div#"+cuid+"_d").removeClass("hidden");
            });

            $("button#changepass").click(function() {
                $("div.container_popup").children("div").addClass("hidden");
                $("div#"+cuid+"_f").removeClass("hidden");
                effect("div#"+cuid+"_f");
            });
            ...
            $("button#newpasschange").click(function() {
                ...
                var value = jQuery.param({"id":id, "password":sha256(password), "chpassword":sha256(new_password)});
                $.ajax({
                    type : "POST",
            ...
            $("div#"+cuid+"_f").focusin(function(e) {
                ...
                if(idx==0||idx==2) {
                    $("div#"+cuid+"_f").animate({"padding":"1% 0 0 0"}, 400);
            ...
            $("div#"+cuid+"_g").focusin(function(e) {
                if($(e.target).nodeName=="INPUT") {
                    $("div#"+cuid+"_g").animate({"padding":"2% 0 0 0"}, 400);
                }
            });
        ...
        //        $(" div.profile").css({"background-image":"url('"+changeinfo+"')"});
                ...
                $("div#"+cuid+"_g > div.profile").css({"background-image":"url('media/user/"+opener.soundgramApi.user_id+"/"+changeinfo+"')"});
                ...
                success: function(data) {
                    if(data.returnCode=="success") {
                        if(type=="profile") {
                            ...
                            $("div#"+cuid+"_g > div.profile").css({"background-image":"url('media/user/"+opener.soundgramApi.user_id+"/"+data.profile+"')"});
