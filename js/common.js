...
function goAuthNext() {
    ...
    // var bodyload = window.setInterval(function() {
        // if(soundgramApi.bookletSize==$("div#booklet_img_list").children().length) {
            // window.clearInterval(bodyload);
            // $("body").show();
            if(soundgramApi.tot_id!="0") {
                // 로딩화면 히든처리
                $(".ci_container").addClass("hidden");
            }
            else {
                $("body").show();
            }
            ...
            //     window.clearTimeout(body_child);
                ...
                var _pm = window.setInterval(function() {
                    if(soundgramApi.pushMovingPage.length>0) {
                        window.clearInterval(_pm);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(returnCode=="complete") {
                ...
                var url = "media/popup/"+soundgramApi.albumid+"/"+data[0].popup_img;
...
function first_loading_animate() {
    ...
    var _hsi = window.setInterval(function() {
        if(ho!="") {
            window.clearInterval(_hsi);
...
function musicSetting(valueofclick) {
    ...
    var play_artist = valueofclick.split("|")[4];
...
function goLoginPage() {
    ...
    if(soundgramApi.tot) {
        loginurl = soundgramApi.serverUrl+"/login_popup_tot.php?device_app_id="+soundgramApi.device_app_id+"&tot_id="+soundgramApi.tot_id+"&uuid="+soundgramApi.uuid+"&diskpath="+soundgramApi.diskpath+"&diskid="+soundgramApi.diskid+"&isiplayer=false&ostype="+soundgramApi.ostype;
    }
...
function profile_detail(user) {
    ...
    else if(user.profile.indexOf("http")!=-1) {
...
function snsSetting(sns) {
    ...
    if($("div.sns_horizontal").is(":visible")) {
        if($("div#snslist").children().length>0) {
            sh_swiper.destroy(true,false);
            $("div#snslist").empty();
        }
    ...
    var tw = window.setInterval(function() {
        if(twitter_url!="") {
            ...
            console.log(soundgramApi.tot)
            if(!soundgramApi.tot) {
                soundgramApi.tplImgNum = "02";
            }

            $("div.sns_bar_icon > img").attr("src",soundgramApi.diskpath+"/"+soundgramApi.diskid+"/images/sn"+soundgramApi.tplImgNum+"_ic_twt_1.png");
...
function logout(snstype) {
    ...
    var logout_api = "api/logout.php";
    if(soundgramApi.tot) {
        value = jQuery.param({"device_app_id":soundgramApi.device_app_id});
        logout_api = "api/logout_tot.php";
    }
        ...
        success: function(data) {
            ...
            if(data.returnCode=="success") {
                if(type=="profile") {
                    ...
                    $("div#"+rv+" > div.review_write > input#contents").text("").val("");
                    // WriteButtonClear(rv);
...
function WriteButtonMake(rv_id , type) {
    ...
    + "<input id='contents' placeholder='댓글을 입력해 주세요.' style='width:72%' autofocus/>"
...
function dropout() {
    ...
    var dropout_api = "api/dropout.php";
    if(soundgramApi.tot) {
        value = jQuery.param({"userid":userid, "device_app_id":device_app_id});
        dropout_api = "api/dropout_tot.php";
    }
        ...
        , success: function(data) {
            ...
            $.ajax({
                ...
                , success: function(data) {
                    ...
                    value = jQuery.param({"uuid":soundgramApi.uuid, "albumid":soundgramApi.albumid});
                    var autologin_api = "api/autologin.php";
                    if(soundgramApi.tot) {
                        value = jQuery.param({"device_app_id":soundgramApi.device_app_id});
                        autologin_api = "api/autologin_tot.php";
                    }
    ...
    confirm.show({
        ...
        btns: [{
            callback: function(instance){
                ...
                else if(type.indexOf("deleteComment")!=-1) {
                    var id = type.split("|")[1];
                    goDeleteComment(id);
                }
...
function tracklistRefresh(device_id, albumid, ilogin, tot_id) {
    // 네이티브 플레이어에서 로그인을 하게되었을 경우에만 수행
    if(ilogin=="true") {
        // 이미 로그인된 상태이면 로그인 로직 타지 않도록..
        var value = jQuery.param({"device_id": device_id, "albumid": albumid, "tot_id":tot_id});
        $.ajax({
            type : "POST",
            url : "api/ilogin.php",
            data : value,
...
// 댓글 삭제 기능 추가 22.04.08
function deleteComment(id,user_id) {
    // 본인이 쓴 댓글의 경우에만 팝업 뜨도록..
    if(soundgramApi.user_id==user_id) {
        showConfirm("deleteComment|"+id, "알림", "댓글을 삭제 하시겠습니까?");
    }
}

// 실제 댓글 삭제 로직 22.04.08
function goDeleteComment(id) {
    var value = jQuery.param({"id":id});
    $.ajax({
        type : "POST",
        ...
        success: function(data) {
            if(data.returnCode=="true") {
                ...
                if($("div.leftmenu_horizontal_swiper").is(":visible")) {
                    ...
                    var _vid = $("div.video_title").attr("value");
                    var videoid = $("div.video_title").attr("value2");
                    videoCommentListSetting("true", _vid, videoid, "");
                }
            }
            else {
...
// 답글 입력
function insertRefComment(obj,type,id,contents_id) {
    var contents = "";
    var contents_type = "4";
    ...
    if(type=="video") {
        contents = $("div#"+soundgramApi.videoPopup+" > div.review_write > input#contents").val();
        if(contents.length==0) {
            alert("댓글을 입력해주세요.");
            return;
        }
        ...
        contents_type = "6";
        contents = $(obj).parent().children(".r_contents").val();
        if(contents.length==0) {
            alert("답글을 입력해주세요.");
            return;
        }
    ...
    var value = jQuery.param({"user_id":soundgramApi.user_id, "albumid":soundgramApi.albumid, "contents_id":contents_id, "contents_type":contents_type, "ref_comment_id":id, "type":type, "contents":contents});
    $.ajax({
        url: "api/insert_ref_comment.php",
        data: value,
        dataType: "json",
        type: "POST",
        success: function(data) {
            if(data.length>0) {
                if(type=="video") {
                    ...
                    // 답글 리스트 초기화 후 생성
                    $(obj).parent().children().not(".r_contents").not(".btn_write").empty();
                    $.each(data, function(al) {
                        var makeListHTML = makeHTML(soundgramApi.diskid.substring(0,1), "refcomment", data[al], "");
                        $(obj).parent().append(makeListHTML);
                    });
                    
                    // 답글 내용 초기화
                    $(obj).parent().children(".r_contents").text("");
                    $(obj).parent().children(".r_contents").val("");
                    
                    // 답글 카운트 업데이트
                    var _cnt = parseInt($(obj).parent().parent().children(".review_content_right").children(".rv_reply").attr("value2"))+1;
                    $(obj).parent().parent().children(".review_content_right").children(".rv_reply").attr("value2",_cnt)
                    $(obj).parent().parent().children(".review_content_right").children(".rv_reply").html("답글 <span class='r_rv_count'>"+_cnt+"</span>");
