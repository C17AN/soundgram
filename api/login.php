<?php
	...
	$tot = $_REQUEST["albumid"]==""?"false":$_REQUEST["albumid"];
	...
	if(empty($user_id)) {
		...
		$diffloginUpdateQuery = "update APIServer_device set user_id=null, login_status=0, login_last_time=null where user_id='$user_id' ";
		$update_result = sql_query($diffloginUpdateQuery);

		$lastloginUpdateQuery = "update APIServer_device set user_id='$user_id', login_status=1, login_last_time=NOW() where device_uuid='$uuid' ";
		...
		$query 
		= " select (select max(id) from APIServer_device where user_id='$user_id') as device_id, id, user_name, user_phone, user_img, user_nick, date_format(reg_time,'%Y-%m-%d') as reg_time, login_type, login_id from APIServer_user where id='$user_id'
		";
