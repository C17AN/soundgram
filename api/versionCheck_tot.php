<?php
	...
	$rows = array();
	if($ostype == 1) {
		$selver = "	select google_ver from APIServer_apppage where tot_id='$tot_id'	";
	} else {
		$selver = " select apple_ver from APIServer_apppage where tot_id='$tot_id' ";
	}
	...
	if($vercheck) {
		if($ostype == 1) {
			$selurl = "	select google_url from APIServer_apppage where tot_id='$tot_id'	";
		} else {
			$selurl = " select apple_url from APIServer_apppage where tot_id='$tot_id' ";
		}
