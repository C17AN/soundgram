<?php
	...
	= " select 	a.id, a.album_name, a.album_type, a.chip_img, a.chip_loading, a.album_time, a.shop_url, a.artist_id, b.artist_name
		from 
			(	select id, album_name, album_type, chip_img, chip_loading, album_time, artist_id, shop_url 
				from APIServer_album 
				where tot_type='$tot_id' 
				and id not in(select album_id from APIServer_device where device_app_id='$device_app_id' and nfc_status='1')
				and order_state='7'
			) a, 
	 		(	select id, artist_name from APIServer_artist) b
	...
	$usealbum_query
	= " select 	a.id, a.album_name, a.album_type, a.chip_img, a.chip_loading, a.album_time, a.shop_url, a.artist_id, b.artist_name, c.nfc_active_time
		from 
			(select id, album_name, album_type, chip_img, chip_loading, album_time, artist_id, shop_url from APIServer_album where tot_type='$tot_id') a, 
			(select id, artist_name from APIServer_artist) b,
			(select id, album_id, nfc_active_time from APIServer_device where device_app_id='$device_app_id' and nfc_status='1') c
		where a.artist_id = b.id and a.id = c.album_id
		order by c.nfc_active_time desc ";
	...
	$rows = array(array($category_rows), array($album_rows), array($usealbum_rows));
