...
<script type="text/javascript">
	...
	function signInWithApple(userIdentifier, fullName, email, uuid, albumid, device_id) {
		...
		console.log(albumid, device_id);
        var profile = "default/profile_none.png";
		
		console.log(fullName, userIdentifier, uuid, device_id, albumid);
		id = `apple_${userIdentifier}`;
        getSNSUser(fullName, userIdentifier, sha256(userIdentifier), "", fullName, 4, uuid, device_id, profile, albumid,"");
	}

	
	function getSNSUser(name, id, password, tel, nick, snstype, uuid, device_id, profile, albumid, access_token) {
        var rcode="";
        var api = "api/getSNSUser.php";
...
<?php
    ...
    $diskpath =     isset($_REQUEST['diskpath'])?$_REQUEST['diskpath']:"";
    $diskid =       isset($_REQUEST['diskid'])?$_REQUEST['diskid']:"";
    $isiplayer =    isset($_REQUEST['isiplayer'])?$_REQUEST['isiplayer']:"";
    $hosturl =      isset($_SERVER['HTTP_HOST'])?$_SERVER['HTTP_HOST']:"";
    $logintype =    isset($_REQUEST['logintype'])?$_REQUEST['logintype']:"";
    $returnCode =   isset($_REQUEST["code"])?$_REQUEST["code"]:""; // 서버로 부터 토큰을 발급받을 수 있는 코드를 받아옵니다.
    $state =        isset($_REQUEST['state'])?$_REQUEST['state']:"";
    $ostype =       isset($_REQUEST['ostype'])?$_REQUEST['ostype']:""; // 21.02.10 iOS에서 소셜로그인 버튼을 누르고 X(닫기)버튼을 누르면 member_close()가 안먹히는 이슈가 있어 추가(조유빈)
    ...
    define('NAVER_CALLBACK_URL', "http://".$hosturl.'/login_popup.php?logintype=naver&albumid='.$albumid.'&device_id='.$device_id.'&uuid='.$uuid.'&isiplayer='.$isiplayer.'&diskpath='.$diskpath.'&diskid='.$diskid); // 네이버 로그인 접근토큰 요청 예제 
    ...
    if($logintype!=null) {
        ...
        if($logintype=="kakao") {
            ...
            $callbacURI = urlencode("http://".$hosturl."/login_popup.php"); // 본인의 Call Back URL을 입력해주세요
            // API 요청 URL
...
?>
                <script type="text/javascript">
                    ...
                    // 회원가입인지 로그인인지 체크
                    getSNSUser(name, id, password, tel, nick, 1, uuid, device_id, profile, albumid, access_token);
                </script>
<?php
            }
        }
...
?>
                    ...
                    // 회원가입인지 로그인인지 체크
                    getSNSUser(name, id, password, tel, nick, 2, uuid, device_id, profile, albumid, access_token);
                </script>
<?php
                } 
                else { 
            ...
            define('GOOGLE_REDIRECT_URL', 'https://devdisk.soundgram.co.kr/login_popup.php');
...
?>
                <script type="text/javascript">
                    ...
                    // 회원가입인지 로그인인지 체크
                    getSNSUser(name, id, password, tel, nick, 5, uuid, device_id, profile, albumid, access_token);
                </script>
<?php
...
<head>
    ...
    <script type="text/javascript">
        ...
        var profile = '<?=$profile?>';
        var access_token = '<?=$access_token?>';
        // var cuid = opener.soundgramApi.cuid;

        // 통합앱 관련 파라미터값
        // var tot_id = opener.soundgramApi.tot_id;
        // if(tot_id.length>0) {
        //     albumid = "tot";
        // }
        ...
        var cuid = "";
        if(isiplayer=="true") {
            var value = jQuery.param({"tot_id":tot_id,"albumid":albumid});
            $.ajax({
                type : "POST",
                url : "api/getcuid.php",
                data : value,
		...
		function EventGather() {
            ...
            $("div#"+cuid+"_b > div.mb_content > ul > li > a").click(function() {
                ...
                if(id=="kakao") {
                    ...
                    var param = ["kakao",albumid,device_id,uuid,isiplayer,diskpath,diskid];
                    ...
                    = "https://accounts.google.com/o/oauth2/auth?client_id="+
                        ...
                        "&redirect_uri=https://devdisk.soundgram.co.kr/login_popup.php"+
                        "&state="+param.join(",")+ 
